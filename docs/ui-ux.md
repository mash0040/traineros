...
(One decision in that paragraph was **undone by #145**: the editor originally refused to clear a weight to bodyweight, showing "To change this to bodyweight, remove the set and log it again without a weight." That refusal was honest about the API rather than about the workflow — `weight_kg: null` meant "leave it alone", so the request would have succeeded and changed nothing. It was also the delete-and-re-add workaround this editor exists to remove, reappearing inside the editor. #145 made absent and explicitly-null different requests, so an emptied weight field clears the column and there is nothing left to refuse.)
