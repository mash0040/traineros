...
**GET /api/clients/:id/sessions was left with no consumer.** #142 moved the client detail screen to /history; the roster was the only other caller. It was recorded as a deletion candidate rather than removed here, because removing a route is an API decision and this ticket scoped one field. **#147 deleted it**, so the route, `ClientSessionResponse` and its generated type are all gone.)

(Recorded in #142, and **overtaken by #115 and #147**: the trainer's view of a client's logged sets is its **own route** rather than sets nested into GET /api/clients/:id/sessions, because at the time that route had a second consumer — the roster read it once per client for the last-session date, and it already downloaded every session a client had ever logged to render one. Nesting sets would have multiplied that by every set, for every client on the roster.

That premise did not survive its own consequences. #115 moved the roster onto `ClientResponse.last_session_on`, which took away the second consumer #142 had been designing around; #147 then deleted the route, since #142 had already taken the first one. The decision was still right — a nested response would have been a third representation of the same rows (below) — but the reason it names is gone, so it is recorded here as history rather than as a live constraint.
...
(Recorded in #29: schedules can be created and edited for deactivated clients. Sending is guarded at worker time per notifications.md, so schedule state and client activation state are independent concerns; deactivation still flips enabled=false in-transaction per #25. The soft/cycling model of client activation justifies keeping them separable. TimeOnly on the wire serializes as HH:mm:ss per framework default. **#79 resolved the input side by measuring rather than converting:** the issue asked for a JsonConverter<TimeOnly> accepting HH:mm, on the premise that the default required seconds — true when this smoke test hit it, and not true on .NET 8, whose built-in reader takes HH:mm, HH:mm:ss and a fractional part. A converter would have *narrowed* what the API accepts (the stock reader also takes "7:00"; a TryParseExact list does not) and changed the sub-second output shape, both for no gain, so none was written. What the ticket actually removed was the SPA-side workaround #51 had built against the stale premise, and what it added was the tests that pin the format so its absence is a decision rather than a drift. Responses still carry seconds, which is why `scheduleTime.ts` keeps `toInputTime`: `<input type="time">` renders empty on a value it cannot parse, so an existing schedule handed one with seconds would present as unset. One schedule per client is app-enforced (no partial unique index); race window is effectively zero at v1 scale, tracked as post-v1 hardening.)
...
Also recorded, and **resolved by #145**: `weight_kg: null` on PATCH used to mean "leave it alone", so a weighted set could not be corrected to bodyweight through this route, and the log screen refused that one edit itself rather than sending a request that would succeed and change nothing. Null now clears the column, so the correction goes through like any other and the client-side refusal is gone. See §Cross-cutting for the rule this is one instance of.)
...
(Recorded in #96: PATCH /api/me/sessions/:id exists because the comment is authored *during* a workout while the row is created at its start (the first logged set), so POST is no longer the moment the note is finished. It accepts `comment` only — `performed_on` and `program_day_id` are what the session is, and changing either makes it a different session rather than an edited one. Extending #32's rule, the window is measured against `workout_sessions.created_at`, the entry event, not `performed_on`; a retroactively logged workout is therefore annotatable through the day it was entered. Divergence from the set patch, where a null field meant "leave it alone": with one field in the body that reading made the request a no-op and left no way to retract a note, so the body was the new value and null or blank cleared it. **Withdrawn by #145**, which gives absence and null separate meanings everywhere and so removes the premise; this route now follows the general rule and an absent `comment` leaves the note alone. A body carrying `performed_on` or `program_day_id` is refused whole with a 400 under the unknown-fields rule rather than silently narrowed to the comment.)
...
- **Error codes are split only where a caller can reach the difference (#124).** `bad_request` covers 61 distinct validation failures (46 explicit sites, 15 from #145's `RejectNull`, plus the body-binding 400), and #124 proposed a per-field code for each. The measurement it asked for is what stopped that: **two are reachable.** Every trainer form validates the same condition before it sends, so a `bad_request` only arrives where the two layers genuinely disagree. The rest divide into a route with no SPA caller at all (password login, since #20 made magic links cover the trainer), values that come from a `<select>` or from the screen's own state, guards mirrored on both sides where a divergence is a bug rather than a design, and #145's "cannot be null" family, which the SPA never sends. So two codes were added and 59 sites keep the shared one.

  - **`invalid_email`** (POST /api/clients). The SPA's `looksLikeEmail` is deliberately looser than `EmailAddresses.IsValid` (#114), so `ada@localhost`, `ada@example..com` and their family pass the form and are refused here. **The message is unchanged and the SPA deliberately does not map this code** — the server's sentence is already written for a person and is word-for-word the SPA's own.
  - **`unknown_timezone`** (POST and PATCH /api/clients). The only validation on this endpoint a trainer can reach without doing anything wrong: the SPA's picker is filled from the *browser's* ICU database via `Intl.supportedValuesOf('timeZone')` while this API validates against the *host's* tzdata, so the two can disagree about a zone that is real and correctly spelled. Mapped, on the `unknown_exercise` argument: the trainer picked it from a list the screen drew, so only the SPA can say what to do instead. Narrowing the picker at source rather than answering after the attempt is tracked as #152, and is not the cheap change it looks: `TimeZoneInfo.GetSystemTimeZones()` returns *Windows* ids on a Windows host and IANA ids on Linux, so an endpoint exposing the server's list would speak a different vocabulary in dev than in prod, and the Windows-to-IANA conversion that would fix that is lossy (~141 entries against the browser's ~400). The likeliest real answer there is keeping the deployment's tzdata current, which needs no API surface at all.

  **The codes are for attribution, not copy, and that is the part worth carrying forward.** The add-client form marks the field a rejection is about; it used to infer that from `bad_request`, which stood for both fields, so a rejected timezone marked the *email* input `aria-invalid` and sent a screen reader user to fix an address that was fine. That is #114's defect surviving on the server-side path. Splitting a code is worth it when a caller has to tell two failures apart, and mostly not otherwise.

- **PATCH semantics (#145): absent, null and a value are three different requests.** A field left out of the body leaves its column alone. A field sent as `null` **clears** it, or is a 400 `bad_request` naming the field if the column is NOT NULL. A field sent with a value writes it. This holds on all nine partial-update PATCH routes and all 28 of their fields, with no per-route exceptions.

(The distinction is already in the JSON — `{"weightKg": null}` and `{}` are different documents — and was being discarded because a `decimal?` on a request record cannot express it. `Patch<T>` in `TrainerOS.Api/Patch.cs` is the reading mechanism; it is mapped out of the OpenAPI document, so the wire and the generated TypeScript are unchanged by it.

**This replaces #26's blank-string sentinel**, under which `""` cleared a nullable *text* column. Two reasons it is retired rather than kept alongside. It only ever worked on strings, which is precisely why `weight_kg`, `rest_seconds` and `starts_on` — the three non-string nullable columns — could be set and never cleared; and keeping it would make `""` and `null` two spellings of one thing on five fields and nowhere else. `""` now means an empty string and is stored as one, which also makes a deliberately-empty cue expressible and distinguishable from an absent one. Trimming stays on every string, because trimming is normalization rather than a sentinel. The create paths were retired with it, so `""` does not mean one thing on POST and another on PATCH.

**It also replaces #96's divergence** on PATCH /api/me/sessions/:id, and removes the reason for it: that route read the body as *the new value* with no presence check, because with null meaning "leave alone" everywhere else there would otherwise have been no way to retract a comment. Two of its four inputs change as a result — an absent `comment` now leaves the note alone rather than wiping it, and a blank one stores `""` — and neither is reachable from the SPA, which always sends the field. Explicit null still clears.

**Null on a NOT NULL column is a 400, where it used to be a silent no-op.** Nothing in the SPA sends one. Worth stating because the failure it replaces was invisible: the caller got a 200 and no field changed.)
